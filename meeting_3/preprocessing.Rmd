...
- \( V \) be the total number of distinct words (vocabulary size).
...
\[
\hat{p}_{g,w} \;=\; \frac{c_{g,w} + \alpha}{\sum_{w} c_{g,w} + \alpha \, V}.
\]
...
\[
\text{log\_odds}_{g,w} \;=\; \log\!\Bigl(\frac{\hat{p}_{g,w}}{1 - \hat{p}_{g,w}} \Bigr),
\]

and then compute the difference:

\[
\Delta_w \;=\; \text{log\_odds}_{1,w} \;-\; \text{log\_odds}_{2,w}.
\]
